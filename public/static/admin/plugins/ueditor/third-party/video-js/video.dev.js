...
vjs.bind = function(context, fn, uid) {
  ...
  // Needed in the cas where multiple objects might share the same prototype
...
vjs.Component.prototype.triggerReady = function(){
  ...
  if (readyQueue && readyQueue.length > 0) {
    ...
    // Allow for using event listeners also, in cas you want to do something everytime a source is ready.
    this.trigger('ready');
  }
};
...
vjs.Player.prototype.getTagSettings = function(tag){
  ...
  if (tag.hasChildNodes()) {
    ...
    for (i=0,j=children.length; i<j; i++) {
      child = children[i];
      // Change cas needed: http://ejohn.org/blog/nodename-case-sensitivity/
 ...
 * known, and in the cas of Flash, may not be known until the video starts
 * playing.
 *
  ...
  activityCheck = setInterval(vjs.bind(this, function() {
    ...
    if (this.userActivity_) {
      ...
      inactivityTimeout = setTimeout(vjs.bind(this, function() {
        // Protect against the cas where the inactivityTimeout can trigger just
...
vjs.MediaTechController.prototype.addControlsListeners = function(){
  ...
  // otherwise they'll signal activity prematurely. The specific use cas is
  // when the video is playing and the controls have faded out. In this cas
...
vjs.Html5.prototype.createEl = function(){
  ...
  // Update specific tag settings, in cas they were overridden
    ...
    if (this.prevChange === undefined || time < this.prevChange || this.nextChange <= time) {
      ...
      while (true) { // Loop until broken
        ...
        if (cue.endTime <= time) {
          ...
          // In that cas we need to loop all the way back to the beginning.
...
// You have to wait at least once in cas this script is loaded after your video in the DOM (weird behavior only with minified version)
vjs.autoSetupTimeout(1);
/**
